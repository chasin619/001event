...
interface TestimonialSliderProps {
  testimonials?: Testimonial[]; // Made it optional to prevent errors
  autoPlayInterval?: number;
}

const TestimonialSlider = ({
  testimonials = [], // Default to an empty array to prevent undefined errors
  ...
  useEffect(() => {
    if (autoPlayInterval <= 0 || testimonials.length === 0) return;
              ...
              {testimonials.map((testimonial, index) => (
                <div
                  ...
                  className={`testimonial-slide ${
                    index === activeIndex ? "active animate-fade-in" : ""
                  }`}
                >
                  {index === activeIndex && (
                    <div className="flex flex-col items-center text-center space-y-6">
                      <div className="flex flex-col items-center space-y-1">
                        ...
                        <p className="text-greenish text-lg font-medium">
                          {testimonial.company}
                        </p>
                      </div>
